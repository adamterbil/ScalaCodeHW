...
object fp3 {
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    ...
    xs match {
      case Nil => true
      case y::ys => allEqualAux(y, xs)
    }
  }
  
  def allEqualAux(a:Int, xs: List[Int]) : Boolean = {
    xs match {
      case Nil => true
      case y::ys => ((a == y) && allEqualAux(a, ys))
    }
  }
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    ...
    xs match {
      case Nil => Nil
      case y::ys => y::removeDupes1Aux(y,ys)
    }
  }

  def removeDupes1Aux[X] (a:X, ys:List[X]) : List [X] = {
    ys match {
      case Nil => Nil
      case z::zs  => {
        if(z == a){
          removeDupes1Aux(z,zs)
        } else {
          z::removeDupes1Aux(z,zs)
        }
      }
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    ...
    xs match {
      case Nil => Nil
      case y::ys => removeDupes2Aux(1, y, ys)
    }
  }

  def removeDupes2Aux [X] (count: Int, a: X, ys: List[X]) : List[(Int, X)] = {
    ys match {
      case Nil => (count, a) :: Nil
      case z::zs  => {
        if(z == a){
          removeDupes2Aux(count + 1, z, zs)
        } else {
          (count, a) :: removeDupes2Aux(1, z, zs)
        }
      }
    }
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    ...
    xs match {
      case Nil => (Nil, Nil)
      case y::ys => splitAtAux(0, n, xs, Nil, Nil)
    }
  }

  def splitAtAux [X] (count:Int, n:Int, xs:List[X], first:List[X],sec:List[X]) : (List[X], List[X]) = {
    xs match {
      case Nil => (first.reverse, sec.reverse)
      case y::ys => {
        if(count < n){
          splitAtAux(count + 1, n, ys, y::first, sec)
        } else {
          splitAtAux(count + 1, n, ys, first, y::sec)
        }
      }
    }
  }
